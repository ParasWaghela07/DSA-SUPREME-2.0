...
-------- MAX PROFIT BY INFINITE TRANSACTIONS WITH COOLDOWN ----------
...
public:
    int byRec(vector<int>&p,int index,int buy){
        ...
        if(buy){
            ...
            int op1=p[index]+byRec(p,index+2,1);
    ...
    int byMem(vector<int>&p,int index,int buy,vector<vector<int>>&dp){
        ...
        if(buy){
            ...
            int op1=p[index]+byMem(p,index+2,1,dp);
