...
public: 
    ...
    int largestIsland(vector<vector<int>>& grid) { O(n^4)
        int ans=0;
        int n=grid.size();
        int m=grid[0].size();
        ...
        for(int i=0;i<n;i++){ O(n^2)
            for(int j=0;j<m;j++){
                ans=max(ans,bfs(grid,i,j)); O(n^2)
            }
        }
...
public:
    ...
    int bfs(vector<vector<int>>&grid,int i,int j,vector<vector<bool>>&vis,map<pair<int,int>,int>&parent,
    int island){
        int n=grid.size();
        int m=grid[0].size();
    ...
    int largestIsland(vector<vector<int>>& grid) {
        ...
        unordered_map<int,int>islandToSum;
        ...
        for(int i=0;i<n;i++){
            for(int j=0;j<m;j++){
                if(!vis[i][j] && grid[i][j]==1){
                    island++;
                    int currsum=bfs(grid,i,j,vis,parent,island);
                    islandToSum[island]=currsum;
                }
            }
        }

        if(parent.size()==0) return 1;
        if(parent.size()==n*m) return n*m;

        // for(auto it:islandToSum){
            ...
            for(int j=0;j<m;j++){
                if(grid[i][j]==0){
                    ...
                    for(int k=0;k<4;k++){
                        ...
                        if(parent.find({i_,j_})!=parent.end() && st.find(parent[{i_,j_}])==st.end()){
                            temp+=islandToSum[parent[{i_,j_}]];
                            st.insert(parent[{i_,j_}]);
                        }
                    }
                    // cout<<"temp : "<<temp<<endl;
                    // cout<<i<<" "<<j<<endl<<endl;
